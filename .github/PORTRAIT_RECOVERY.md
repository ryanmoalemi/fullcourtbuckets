...
Confirm the actual error and schema. For a newly generated image, preserve it to Drive using the runtime-generated file reference returned by image generation (for example the `file_...` reference) as the connector `file_uri`; do not substitute the mounted `/mnt/data/...` filesystem path when the destination schema expects a file reference. The Aneesah Morrow recovery proved that the generated runtime file reference can be accepted even when the mounted path is rejected. If the runtime file reference succeeds, classify the earlier BLOCKED_FILE_REFERENCE as a file-reference/schema mismatch rather than a security denial.
...
If a newly generated image cannot be preserved to Drive after the exact runtime `file_...` reference is tried once, publication may continue through the supported temporary Google Doc `insertInlineImage` handoff by passing that SAME runtime `file_...` reference in both the request `uri` placeholder and `image_uris`. Do not substitute the mounted `/mnt/data/...` path for this recovery path. Read the exact returned `imageProperties.sourceUri` immediately. The existing uploader must receive that sourceUri, validate the full original, and preserve the exact bytes in `portrait-masters/<player_id>-<slug>-<sha256>.png` in GitHub before publication. This is the approved Drive-independent fallback. Do not use `contentUri`, guessed URLs, screenshots, recompression, or base64 chunking.

BLOCKED_FILE_REFERENCE is NOT automatically a stale-path error. First distinguish a wrong file-reference form from a real security/egress restriction by checking the destination schema and, where supported, trying the exact runtime file reference once. If the error still explicitly indicates a security, egress or authorization restriction, stop that denied operation. Do not evade the restriction by copying/renaming the file, changing hostnames, converting signed URLs, using a different endpoint, or relabeling its provenance. Record the restriction and continue only independent work permitted by the tools. Request the smallest necessary owner action when the connector actually requires it.
