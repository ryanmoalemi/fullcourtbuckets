...
1. Verify local approved source bytes and player ID/slug. Prefer preserving the master in connected Drive under a unique player-ID/slug/checksum filename. If Drive rejects the exact runtime `file_...` reference, do not block publication: continue with the temporary-Doc whole-file handoff below. The uploader now preserves the same exact PNG in GitHub under `portrait-masters/<player_id>-<slug>-<sha256>.png` as a durable fallback master.
...
3. Proven supported handoff: create a TEMPORARY native Google Doc; call `batch_update_document` with `insertInlineImage`, setting the request `uri` placeholder and `image_uris` to the exact runtime-generated `file_...` reference. This form was re-verified on 2026-09-19 after mounted `/mnt/data/...` paths returned `BLOCKED_FILE_REFERENCE`. It uses runtime's whole-image upload handling, not base64.
